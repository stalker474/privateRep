...
void UStrategyAIAction_ReturnToBase::Activate()
{
	...
	AActor* Actor = pController->GetPawn();
	if (Actor != NULL && Actor->GetOwner() != NULL)
	{
		bIsMoving = true;
		Destination = Actor->GetOwner()->GetActorLocation();
		auto res = MyAIController->MoveToLocation(Destination, 10.0f, true, true, true);
	}
...
void UStrategyAIAction_ReturnToBase::OnMoveCompleted()
{
	...
	AActor* Actor = pController->GetPawn();
	Actor->SetActorRotation(Actor->GetOwner()->GetActorRotation());
}
