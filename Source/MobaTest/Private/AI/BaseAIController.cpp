...
void ABaseAIController::UnClaimAsTarget(TWeakObjectPtr<AController> InController)
{
	for (int32 Idx = ClaimedBy.Num() - 1; Idx >= 0; Idx--)
	{
		...
		const AMobaPlayerController* TestD = Cast<AMobaPlayerController>(ClaimedBy[Idx].Get());
		if (TestC != NULL && TestC == InController.Get())
		{
			ClaimedBy.RemoveAt(Idx);
		}
		else if (TestD != NULL && TestD == InController.Get())
		{
			ClaimedBy.RemoveAt(Idx);
		}
	}
...
void ABaseAIController::Tick(float DeltaTime)
{
	...
	if (!IsLogicEnabled() || (GetPawn() == NULL) || (pMobaAI != NULL && pMobaAI->CombatComponent->Health <= 0))
	{
		if (!IsLogicEnabled())
		{
